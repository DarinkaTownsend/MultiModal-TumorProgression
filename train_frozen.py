...
def main(args):
    ...
    trainLoader = DataLoader(trainSet, batch_size=args.batchSize, shuffle=True,
                             num_workers=4, collate_fn=collateFn, pin_memory=True)
    ...
    # freeze pretrained encoders — only train fusion + head
    for param in model.imgEncoder.vit.parameters():
        param.requires_grad = False
    for param in model.txtEncoder.bert.parameters():
        param.requires_grad = False

    trainableParams = sum(p.numel() for p in model.parameters() if p.requires_grad)
    totalParams     = sum(p.numel() for p in model.parameters())
    print("trainable params: %d / %d" % (trainableParams, totalParams))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if f1 > bestF1:
            bestF1    = f1
            noImprove = 0
            torch.save(model.state_dict(), os.path.join(args.outputDir, "best_model_frozen.pt"))
            print("  saved best model (f1=%.4f)" % bestF1)
        else:
            noImprove += 1
            print("  no improvement (%d/%d)" % (noImprove, patience))
    ...
    model.load_state_dict(torch.load(os.path.join(args.outputDir, "best_model_frozen.pt")))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest",    default="/home/ob675831/tumor_progression/processed/manifest.csv")
